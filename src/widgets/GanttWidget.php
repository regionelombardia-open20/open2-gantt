...
JS;
        ...
        $confirmMessageActivity = \Yii::t('amosgantt',  "Sei sicuro di andare alla pagina di modifica dell\'attività?");
        $confirmMessageWp = \Yii::t('amosgantt',  "Sei sicuro di andare alla pagina di modifica del WP?");

        $jsCustom = <<<JS
$(document).on('click', ".gantt_bars_area .gantt_task_line, .gantt_grid_data .gantt_row", function() {
    var id = $(this).attr('task_id');
    var confirmMex = '';
    if(id.search('p') >= 0){
         confirmMex = '$confirmMessageWp'

    }else {
         confirmMex = '$confirmMessageActivity'
    }
     var task = gantt.getTask(id);
        if(confirm(confirmMex)){
